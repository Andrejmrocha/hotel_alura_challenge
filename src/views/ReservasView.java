...
public class ReservasView extends JFrame {
			...
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtDataE.getDate() != null && ReservasView.txtDataS.getDate() != null) {
					if(txtValor.getText().contains("-")) {
						JOptionPane.showMessageDialog(null, "Seu check out não pode ser antes do check in.");
					}
					else {
						salvarReserva();
					}
	...
	//Código que permite movimentar a janela pela tela seguindo a posição de "x" e "y"	
	 ...
	 private void calcularValor(JDateChooser entrada, JDateChooser saida) {
		 if(entrada.getDate() != null && saida.getDate() != null) {
			 Calendar inicio = entrada.getCalendar();
			 Calendar fim = saida.getCalendar();
			 int diaria = 100;
			 int dias = -1;
			 int valor;
	 ...
	 private void salvarReserva() {
		 ...
		 Reserva reserva = new Reserva(Date.valueOf(dataEntrada), Date.valueOf(dataSaida),
				 txtValor.getText(), txtFormaPagamento.getSelectedItem().toString());
		 reservaController.cadastrar(reserva);
		 
		 JOptionPane.showMessageDialog(contentPane, "Registro salvo, Número de reserva: " + reserva.getId().toString());
		 RegistroHospede registroHospede = new RegistroHospede(reserva.getId());
